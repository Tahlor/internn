...
class AutoStat(Stat):
    def __init__(self, counter_obj, x_plot="epochs", x_weight="instances", x_title="", y_title="", name="", plot=True, ymax=None, train=True, **kwargs):
        """ AutoStat - same as Stat, but don't need to specify the x-coords every time
            Specify the x_weight once, which should be a STRING to reference the counter object (not an actual number) (e.g. number of instances, number of datapoints)
    ...
    def accumulate(self, sum, weight=1):
        self.current_sum += sum * weight
        self.accumlator_active = True
        self.weight += weight
    ...
    def reset_accumulator(self, new_x=None):
        if self.accumlator_active:
            ...
            # Reset Accumulator
            self.current_sum = self.weight = 0
            self.accumlator_active = False
            self.updated_since_plot = True
...
class Counter:
    ...
    def update(self, epochs=0, instances=0, updates=0, preds=0):
        self.epochs += epochs
        self.instances += instances # training instance
        self.updates += updates # a batch update
        self.epoch_decimal = self.instances / self.instances_per_epoch
        self.preds += preds # an actual prediction, e.g. a letter

if __name__=='__main__':
    training_counter = Counter()
    training_counter.epochs += 1
    print(training_counter.epochs)
    a = AutoStat(counter_obj=training_counter)
    a.accumulate(55)
    a.accumulate(65)
    a.reset_accumulator()
