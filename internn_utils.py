...
def load_config(config_path, hwr=True,
    ...
    # If using preloaded model AND override is not mentioned
        ...
        _output = incrementer(Path(config["load_path"]).parent,
                              "new_experiment")  # if it has a load path, create a new experiment in that same folder!
    ...
    if config.dataset.include_synthetic:
        if config.TESTING:
            config.dataset.extra_dataset = [
                "online_coordinate_data/MAX_stroke_vFullSynthetic100kFull/train_online_coords_sample.json"]
        else:
            config.dataset.extra_dataset = [
...
def validate_and_prep_loss(config):
    ...
    for loss_fn_group in [k for k in config.keys() if "loss_fns" in k]:  # [loss_fns, loss_fns2]
        for i, loss in enumerate(config[loss_fn_group]):  # [{name: , coef: } ...]
            ...
            if not "gts" in loss:
                continue
            indices = [config.gt_format.index(k) for k in
                       loss["gts"]]  # This will throw an error if the loss expected something not in the GT
...
def save_model_stroke(config, bsf=False):
    ...
    if "training_dataset" in config and "dtw_adaptive" in config.all_losses:
        np.save(Path(config["results_dir"]) / "training_dataset.npy",
                [{"gt": gt["gt"], "image_path": gt["image_path"]} for gt in config.training_dataset.data])
...
def load_model_strokes(config, load_optimizer=True, device="cuda"):
    ...
    if "scheduler" in old_state and load_optimizer and "scheduler" in config:
        ...
        config.scheduler = new_scheduler(config.optimizer, old_batch_size, gamma=config.scheduler_gamma,
                                         last_epoch=config.counter.updates)
...
def create_resume_training(config):
    ...
    with open(Path(output / 'TEST.yaml'), 'w') as outfile:
        ...
        if (export_config["testing_occlude"] or export_config["testing_warp"]) and not export_config[
            "n_warp_iterations"]:
...
def reset_all_stats(config, keyword="", freq=None):
    ...
    for title, stat in config["stats"].items():
        if isinstance(stat,
                      Stat) and stat.accumlator_active and stat.accumulator_freq == freq and keyword.lower() in stat.name.lower():
        ...
        try:
            ...
            try:
                return self.check_dict(
                    o.default(o))  # hopefully the class has a default function; clean up the dict as needed
            except:
                return o
...
def stat_prep(config):
    ...
    config_stats.append(AutoStat(counter_obj=config.counter, x_weight="test_instances", x_plot="epoch_decimal",
                                 ...
                                 train=False, ymax=.2))
    config_stats.append(
        AutoStat(counter_obj=config.counter, x_weight="validation_pred_length_static", x_plot="epoch_decimal",
...
def stat_prep_strokes(config):
    ...
    config.counter = Counter(instances_per_epoch=config.n_train_instances, test_instances=config.n_test_instances,
                             test_pred_length_static=config.n_test_points)
    ...
    for variant in ("train", "test"):
        ...
        # TOTAL ACTUAL LOSS
        config_stats.append(AutoStat(counter_obj=config.counter, x_weight=x_weight, x_plot="epoch_decimal",
                                     x_title="Epochs", y_title="Loss", name=f"Actual_Loss_Function_{variant}",
                                     train=is_training))
        ...
        for loss in config.all_losses:
            if loss != "l1_default":
                config_stats.append(AutoStat(counter_obj=config.counter, x_weight=x_weight, x_plot="epoch_decimal",
                                             x_title="Epochs", y_title="Loss", name=f"{loss}_{variant}",
                                             train=is_training))
    ...
    if Path(path).suffix == ".json":
        ...
        if numpy_path.exists() and \
                (not Path(path).exists() or \
                 os.path.getmtime(numpy_path) > max(RECREATE_TIME,
                                                    os.path.getmtime(path))):  # if numpy exists and is sufficiently new
